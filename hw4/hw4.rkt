...
(define (list-nth-mod xs n) (cond [(< n 0) (error "list-nth-mod: negative number")]
                                  ...
                                  [ #t (letrec ([i (remainder n (length xs))]
                                                [f (lambda (ys j) (if (= j i) (car ys) (f (cdr ys) (+ 1 j))))])
                                         (f xs 0))]))
...
(define (vector-assoc v vec) (letrec ([l (vector-length vec)]
                                      [f (lambda (i)
                                           (if (= i l)
                                               #f
...
(define (cached-assoc xs n) (letrec ([cac (make-vector n #f)]
                                     [pos 0]
                                     [f (lambda (v) (let ([prc (vector-assoc v cac)])
                                                      (if prc
                                                          prc
                                                          (let ([pr (assoc v xs)])
                                                            (if pr
