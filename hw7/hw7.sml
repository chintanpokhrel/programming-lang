...
fun eval_prog (e,env) =
	...
	(case List.find (fn (s2,v) => s=s2) env of
      ...
      | Shift(dx, dy, e) => let val a = eval_prog(e, env)
			    in
				case a of
...
fun preprocess_prog p =
	...
	Let(s, e1, e2) => Let(s, preprocess_prog e1, preprocess_prog e2)
      ...
      | LineSegment (x1, y1, x2, y2) => let val ans = real_close(x1, x2)
					in
					    if ans andalso real_close(y1, y2)
					    then Point (x1, y1)
					    else if ans andalso y2 < y1
